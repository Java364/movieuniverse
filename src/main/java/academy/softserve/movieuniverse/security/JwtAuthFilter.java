    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (accessToken != null && jwtTokenProvider.validateToken(accessToken)) {
                ...
                /*
                 * Role role = userRepository.findByEmail(email).getRole(); Long id =
                 * userRepository.findByEmail(email).getId(); response.setHeader("Access-token",
                 * jwtTokenProvider.generateAccessToken(id, email, role)); response.setHeader("Refresh-token",
