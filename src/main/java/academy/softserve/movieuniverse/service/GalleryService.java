...
public class GalleryService {
    ...
    public Gallery update(Gallery gallery) {
        if (gallery == null || gallery.getId() == null || !galleryRepository.findById(gallery.getId()).isPresent())
            throw NotFoundException.createNotFoundException(ExceptionType.UPDATE.getMessage() + " gallery");
        gallery = galleryRepository.save(gallery);
        if (gallery == null) throw NotFoundException.createNotFoundException(ExceptionType.UPDATE.getMessage() + " gallery");
        return gallery;
    ...
    public Gallery findById(Long id) {
        ...
        if (!galleryOptional.isPresent()) {
            throw NotFoundException.createNotFoundException(ExceptionType.SELECT.getMessage() + "gallery with " + id.toString() + " ID");
        }
    ...
    public void deleteById(Long id) {
        if (id == null || !galleryRepository.findById(id).isPresent())
            throw NotFoundException.createNotFoundException(ExceptionType.DELETE.getMessage() + "gallery with " + id.toString() + " ID");
